...
!xmaxt = 600.
!xmaxdepth = -7.e3
!ZD= 0.25*(cord(1,nx/2,2)+cord(2,nx/2+1,2)+cord(1,nx/2+1,2)+cord(2,nx/2,2))
do i = 1,nx-1
    do j = 1,nz-1
...
!        print *, 'cp_eff= ', cp_eff, 'cond_eff', cond_eff
      ...
      tmpr = 0.25*(temp(j,i)+temp(j+1,i)+temp(j,i+1)+temp(j+1,i+1))
       yc = 0.25*(cord(j,i,2)+cord(j+1,i,2)+cord(j,i+1,2)+cord(j+1,i+1,2))+&
            0.25*(cord(1,nx/2,2)+cord(2,nx/2+1,2)+cord(1,nx/2+1,2)+cord(2,nx/2,2)) 
!   print *, xmaxt,xmaxdepth
      if( tmpr.le.xmaxt .and.yc.ge.xmaxdepth) then
        diff = nusselt*cond_eff/den(iph)/cp_eff
!print *,cond_eff,diff,yc, tmpr 
     else
